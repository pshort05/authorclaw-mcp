...
├── src/
│   ├── server.ts                  # MCP server entry point (SSE + stdio transport)
│   ├── auth/
│   │   └── oauth.ts               # OAuth 2.1 handler (forked from freema/openclaw-mcp)
│   ├── client/
│   │   └── authorclaw.ts          # HTTP client for AuthorClaw REST API (:3847)
│   ├── tools/
│   │   ├── chat.ts                # authorclaw_chat, authorclaw_chat_async
│   │   ├── projects.ts            # authorclaw_project_create / _status / _list / _stop
│   │   ├── files.ts               # authorclaw_files_list / _read / _export
│   │   ├── research.ts            # authorclaw_research
│   │   ├── status.ts              # authorclaw_status (heartbeat)
│   │   └── tasks.ts               # async task queue (ported from freema)
│   └── config.ts                  # env-driven config with AuthorClaw defaults
├── docker/
│   ├── Dockerfile                 # Node 22 slim, non-root user
│   └── docker-compose.yml         # authorclaw + authorclaw-mcp as a stack
├── scripts/
│   └── apply-lan-patch.sh         # copied verbatim from authorclaw-docker-fix
├── docs/
...
| Tools | `openclaw_chat`, `openclaw_status` | 12 author-specific tools |
...
# 3. Copy the LAN patch script from the docker-fix repo.
cp ../authorclaw-docker-fix/apply-lan-patch.sh scripts/

# 4. Install dependencies (freema's package.json is the starting point).
npm install

# 5. Replace src/tools/ with the AuthorClaw-specific tools (§7–§8).
